...
rfmodel_trump <- randomForest(fraction_votes ~ median_income + under18 + over65 + black + asian + latino + white + foreign + college + homeowners + pop_density, data = training, importance=TRUE, ntree=40)
plot(rfmodel_trump)
varImpPlot(rfmodel_trump, type=1)

#predict using fitted model
predicted <- predict(rfmodel_trump, newdata=testing[ ,-1])

#measure out of sample
actual <- testing$fraction_votes
